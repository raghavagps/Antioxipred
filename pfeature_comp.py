...
def SE(filename, out):
    # Read sequences (FASTA already converted to single-column CSV)
    data = list(pd.read_csv(filename, header=None).iloc[:, 0])
    ...
    for i, seq in enumerate(data):
        ...
        if not set(seq).issubset(allowed):
            print(
                "Error: Please check for invalid inputs in the sequence.",
                "\nError in:",
                "Sequence number =", i + 1,
                ", Sequence =", seq,
                "\nNOTE: Spaces, special characters and extra characters (BJOUXZ) are not allowed."
            )
            return

        Val.append(round(entropy_single(seq), 3))

    # ✅ Write CSV ONCE (correct format)
...
def paac(file,lambdaval,out,w=0.05):
    ...
    # Safe delete
    for f in [filename + ".lam", filename + ".aac"]:
        try:
            if os.path.exists(f):
                os.remove(f)
...
#################################Transition#############
    ...
    # === FIX START: Used iloc instead of loc ===
    head = []
    ...
    # Fixed Loop 1 (Using iloc)
    for i in range(0,len(df_1),7):
        rows_to_concat = [df_1.iloc[i+k] for k in range(7)] 
        row_combined = pd.DataFrame(pd.concat(rows_to_concat, axis=0)).transpose()
        zz = pd.concat([zz, row_combined], ignore_index=True)
    ...
    # Fixed Loop 2 (Using iloc)
    for i in range(0,len(df_2),7):
        rows_to_concat = [df_2.iloc[i+k] for k in range(7)]
        row_combined = pd.DataFrame(pd.concat(rows_to_concat, axis=0)).transpose()
        ss = pd.concat([ss, row_combined], ignore_index=True)
    ...
    # Fixed Loop 3 (Using iloc)
    for i in range(0,len(df_3),21):
        rows_to_concat = [df_3.iloc[i+k] for k in range(21)]
        row_combined = pd.DataFrame(pd.concat(rows_to_concat, axis=0)).transpose()
        rr = pd.concat([rr, row_combined], ignore_index=True)
